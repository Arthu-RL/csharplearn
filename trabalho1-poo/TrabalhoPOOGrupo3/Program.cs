...
namespace TrabalhoPOOGrupo3
{
        ...
        public static void Menu()
        {
            while (true)
            {
                ...
                switch (Console.ReadLine())
                {
                    ...
                    {
                        Console.WriteLine(
                            "Digite um valor em metros para ser convertido para milímetros:"
                        );
                        ...
                        int milimeters = milimetros.Milimetros.MetersToMillimeters(meters);
                    ...
                    case "4":
                    {
                        ...
                        if (!Int32.TryParse(cInput, out c))
                            throw new ArgumentException("Valor de C inválido.");

                        double bhaskaraCalc = bhaskara.BhaskaraCalc.Bhaskara(b, a, c);

                        Console.WriteLine($"Δ = b² - 4ac");
                        Console.WriteLine($"Δ = ({b})² - 4 * {a} * {c}");
                        Console.WriteLine($"Δ = {Math.Pow(b, 2)} - 4 * {a} * {c}");
                        Console.WriteLine($"Δ = {Math.Pow(b, 2)} - ({4 * a * c})");
                        Console.WriteLine($"Δ = {bhaskaraCalc}");
                        break;
                    }
    ...
    public class Car
    {
        ...
        {
            bool isNumberOfDaysInvalid = numberOfDays < 1;

            if (isNumberOfDaysInvalid)
